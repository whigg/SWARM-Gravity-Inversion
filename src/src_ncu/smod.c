...
double accel_ac_part (double *tjd, double *xic, double *acc, double *c_si)
{
    double tt, as[3], qvec[4], c_is[9];
...
/*todo: air drag acc & partial to vxvyvz dadv*/
    ...
    accel_ac_part (tjd, xic, ac, c_si);

//    printf ("%e\t%e\t%e\t", ac[0], ac[1], ac[2]);
//    printf ("%e\t%e\t%e\n", an[0], an[1], an[2]);
...
///////////////////////stop here for GRACE w/o acc parameters//////////////////
    ...
    if (MACC >= 3)
    {
        dadp[0 * MDYN + i] = c_si[0]; dadp[1 * MDYN + i] = c_si[3]; dadp[2 * MDYN + i] = c_si[6];
        i++;
        dadp[0 * MDYN + i] = c_si[1]; dadp[1 * MDYN + i] = c_si[4]; dadp[2 * MDYN + i] = c_si[7];
        i++;
        dadp[0 * MDYN + i] = c_si[2]; dadp[1 * MDYN + i] = c_si[5]; dadp[2 * MDYN + i] = c_si[8];
        i++;
    }
...
//        cs2acc (llr, COEFO, GM, radius, NOMAX, ae);
        cs2ada (llr, COEFO, NOMAX, ae, part, dadres, 0);
...
//    stpole (info->mjd, info->xp, info->yp, &c21p, &s21p);
